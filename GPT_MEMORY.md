...
| 批次 | 条数 |
|---|--:|
| `artist_bios_batch02_30_ascii.json` | 30 |
...
A.R. Kane  ADOY  Abdullah Ibrahim  Ada Lea
Adorable  Adrianne Lenker  Ahmad Jamal  Air
Akira Kosemura  Al Green  Alabaster DePlume  Alasdair Roberts
Aldous Harding  Alela Diane  Alex G  Alex Isley
Alexandra Stréliski  Alfa Mist  Ali Farka Touré  Alice Coltrane
Allegra Krieger  Allen Toussaint  Alpha  Altın Gün
Alvvays  Amber Mark  Ambrosia  American Football
Amiina  Ana Frango Elétrico  Ana Roxanne  Anastasia Coope
Anaïs Mitchell  Andrew Bird  Andy Shauf  Angel Olsen
Angela Bofill  Angie Stone  Animal Collective  Anjimile
Ann Peebles  Anna B Savage  Anna Burch  Anna St. Louis
Anna Tivel  Anne Briggs  Anne Müller  Another Sunny Day
...
- **器乐**：194/1169 首（17%），偏低
- **明快上扬**：`upbeat` 64 首 · `hopeful` 91 首，整期容易一路温柔到底
- **年代**：1950s 3 · 1960s 34 · 1970s 101 · 1980s 84 · 1990s 124 · 2000s 173 · 2010s 385 · 2020s 265
- **非英语世界**：日语 / 韩语 / 西语 / 北欧 / 中东 / 非洲
- **BPM**：< 70 的极慢与 > 125 的快都少

当前流派 220 类，TOP10：dream pop(151)、neo soul(117)、organic electronic(115)、art pop(110)、indie rock(99)、indie pop(97)、chamber pop(95)、downtempo(94)、jazz(87)、ambient folk(83)
...
**从 2026-08-03 起，补库与写简介合成一次交付、只上传一次**：一个文件、
顶层对象、两个键，传到 `candidates/`。完整流程见
[`GPT_WEEKLY.md`](GPT_WEEKLY.md)（运行入口，以它为准）。

```python
import json, hashlib

data = {'tracks': [...],      # 本批曲目（字段见 GPT_CATCHUP.md）
        'artists': [...]}     # 本批【新】艺人的简介，已在库的别重复写
s = json.dumps(data, ensure_ascii=True, indent=1)   # ← 关键：ensure_ascii
open('YYYY-MM-DD.json', 'w').write(s)
print(hashlib.sha256(s.encode('ascii')).hexdigest())
```

**新艺人数必须等于简介数** —— 入库曲目的艺人不在上面「已写清单」里，
就必须在同一份文件的 `artists` 里带简介，缺一个整批被拒（会逐个点名）。

> 只补简介、不带曲目时，仍可单独传 `inbox/bios/`（专用通道）。
> 那条通道要配 manifest，名字 = 主文件名去掉 `.json` + `_manifest.json`，
> 一个字都不能差；配不上会硬失败并告诉你它在找哪个名字。
