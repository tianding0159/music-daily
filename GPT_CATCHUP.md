...
> 这份是**增量说明**，只讲变了什么。完整补库提示词仍在 [`REPLENISH_PROMPT.md`](REPLENISH_PROMPT.md)。
> 两份冲突时，**以本文为准**（本文更新在后）。
...
| 3 | 文案黑名单扩到 **47个词**，且**含「空气感」「颗粒感」** | 命中即拒收 |
...
输出**一个JSON数组**，不要markdown围栏、不要弯引号、能通过严格 `json.loads`。

## 7. 交付方式

产出的JSON直接给我（Claude），我负责跑合并管线（iTunes验真 → 去重 → schema/文案/词表校验 → 进池 → 重建部署）。
也可以自己放进repo的 `candidates/` 目录，CI的 `merge.yml` 会自动处理——这条链路已验证正常（你上次补的227首就是这么进来的，媒体命中率227/227）。
