...
你是顶尖独立音乐策展人 + 中文乐评写手。请根据我上传的 `profile.md`（口味）与 `style_bible.md`（文案文风），为我的"每日音乐日报"候选池发现新曲目。**批量约定**：日常手动「补池」约 **50首**即可；每周自动「补库」为 **225首**（内部可按9批 × 25首发现，最终合并成**一个** JSON数组）。数量绝不能靠降低品味/验真/文案标准来凑。
...
7. **原版不得错配版本**：候选是原版就必须是原版；若某曲在流媒体只有Remix / Live / Remaster / Edit / Acoustic等版本、而你没在标题里声明该版本，宁可不收（落地端会判为version_mismatch隔离）。
8. **year必须与所填album的正式发行年份一致**（单曲先行、后收入专辑时，album填专辑则year用专辑年）。
9. 最终必须是**标准JSON**：用ASCII直双引号 `"`、能通过严格 `json.loads`；不要弯引号、不要模板句、不要无明确旋律的ambient。
